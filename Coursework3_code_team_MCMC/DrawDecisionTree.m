...
if ~isempty(tree.kids)
    ...
    text(tree.X, tree.Y, strcat("samples: ", num2str(tree.samples)) ,'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'interpreter', 'none', 'FontSize', 10);
else
    line(tree.X, tree.Y, 'marker', '.', 'markersize', 8)
    if isfield(tree, 'predictionClass') 
        text(tree.X, tree.Y, num2str(tree.predictionClass) ,'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', 'interpreter', 'none', 'FontSize', 10);
    else
