% function for getting the processed dataset, both classification and regression.
% return cFtrs => [matrix] Classification features
%        cLbls => [matrix] Classification labels
%        rFtrs => [matrix] Regression features
%        rLbls => [matrix] Regression labels
% parameter normalization => [boolean] Do normalize to the features or not.
function [cFtrs, cLbls, rFtrs, rLbls] = DataHandling(normalization)
    rng(1);
    ...
    if normalization
        cFtrs(:,1:8) = normalize(cFtrs(:,1:8));
        rFtrs = normalize(rFtrs);
    end
