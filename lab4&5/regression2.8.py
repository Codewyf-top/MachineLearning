...
def multilayer_perceptron(input_d):
    ...
    #add output layer
    output_layer = add_layer(l2, n_hidden2, n_output,n_layer=3, activation_function=None) #in_size=10是l1的outsize 1是y_data的size 如果activation function=None, 就为线性函数
    return output_layer
...
y_tr = y_data[0:1300, np.newaxis]
y = y_data[:, np.newaxis]

# # setting test dataset
x_test = x_data[1301:, :]
# # reshape y to fit the placeholder
y_test = y_data[1301:, np.newaxis]
...
def run(x,y,x_t,y_t,k):
    ...
    with tf.Session() as sess:
        ...
        accuracy = tf.reduce_mean(tf.keras.losses.MSE(pred, ys))
        print("Test loss:", accuracy.eval({xs: x_t, ys: y_t}))
        output = pred.eval({xs: x_t})

        for i in range(x.shape[1]):
            plt.subplot(230 + i + 1)
            plt.plot(x_t[:, i:i + 1], y_t, 'ro', markersize=6, label='Original value')
            plt.plot(x_t[:, i:i + 1], output, 'bo', markersize=4, label='Predicted value')
            plt.legend()
            plt.title(get_features_name(i))
        plt.show()
...
def main():
    ...
    new_x_test =[]
    new_y_test =[]
    run(x_tr,y_tr,x_test,y_test,0)#train
    print("*" * 50)
    kf = KFold(10, False)
    for train_index, test_index in kf.split(x_data):
        ...
        print(f"fold:{fold}")
        fold = fold + 1
        run(new_x_train1,new_y_train1,new_x_test1,new_y_test1,1)
